...
- Rationale: embedders can use a blocking API without managing async runtime details.
...
- Rationale: `ferrule-sql` remains embeddable and config-free.
...
- Rationale: stable exit codes distinguish usage, connection, query, and notable-result outcomes.
...
- Rationale: user commands should not fail because history/cache side effects fail.
...
- Rationale: keep default/backend capability explicit and allow opt-in Oracle/SSH/TUI behavior.
...
- Rationale: database results can be large; failure should be bounded instead of OOM-prone.
...
- Rationale: output and configuration order can be user-visible.
...
- Rationale: mdBook source/output split.
